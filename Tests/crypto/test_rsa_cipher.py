...
def test_rsa_write_chiper_text_and_decrypt(create_file_module, rsa_cipher):
    data = DEFAULT_FILE_CONTENT
    with open(create_file_module.get("file_path"), "wb") as r_file:
        key_filename = rsa_cipher.write_chiper_text(data, r_file, create_file_module.get("file_name"))
