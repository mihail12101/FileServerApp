...
def test_aes_write_chiper_text_and_decrypt(create_file_module, aes_cipher):
    data = DEFAULT_FILE_CONTENT
    with open(create_file_module.get("file_path"), "wb") as r_file:
        key_filename = aes_cipher.write_chiper_text(data, r_file, create_file_module.get("file_name"))

    key_path = os.path.join(aes_cipher.KEY_DIR, key_filename + FILE_EXTENSION)

    assert os.path.isfile(key_path)

    with open(create_file_module.get("file_path"), "rb") as r_file:
